...
class MoveBaseClient():
	...
	def give_goal_cell(self, pose):
		#rospy.loginfo("\n" + str(pose))
		if (pose not in self.goals) and (len(self.goals) < 2):
			self.goals.poses.append(pose)
		
		if (len(self.goals) == 2):
			self.goals.header.stamp = rospy.Time.now()
			self.goals_pub.publish(self.goals)
			self.goals.poses[:] = []
...
if __name__ == '__main__':
	try:
		...
		goal1 = Pose()
		goal2 = Pose()
		MoveBaseClient.give_goal_cell(goal1)
		MoveBaseClient.give_goal_cell(goal2)
		MoveBaseClient.send_first_goal()
